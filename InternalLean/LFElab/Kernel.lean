...
def emitStructuralReplayFallbackStart (theoryName : Name)
    ...
    (statement : Kernel.Judgment) (filterSummary? : Option String := none) : CoreM Unit := do
...
structure StructuralTheoremSchemaFilterStats where
  ...
  considered : Nat := 0
  /-- Considered schemas selected by the active demand set. -/
  demanded : Nat := 0
  /-- Schemas actually lowered by the active filter. -/
  lowered : Nat := 0
  /-- Demanded theorem-schema names, in checked-signature order. -/
  demandedNames : Array Name := #[]
  /-- Lowered theorem-schema names not demanded by the current theorem, in signature order. -/
...
def structuralTheoremSchemaFilterStats (theorems : Array CheckedLFJudgmentTheorem)
  ...
  theorems.foldl (init := {}) fun stats t =>
    if checkedLFJudgmentTheoremNeedsStructuralRuleSchema t then
      ...
      let demanded := filter.demandedTheorems.contains name
...
/-- Primitive rule schemas demanded by direct rule applications in a checked derivation. -/
partial def primitiveRuleAppsInCheckedLFDerivation : CheckedLFDerivation → NameSet
  | .localAssumption .. => {}
  | .theoremRef _ _ _ premises => Id.run do
      let mut refs : NameSet := {}
      for prem in premises do
        refs := mergeStructuralTheoremNameSet refs (primitiveRuleAppsInCheckedLFDerivation prem)
      return refs
  | .ruleApp ruleName _ _ premises _ => Id.run do
      let mut refs : NameSet := {}
      refs := refs.insert ruleName.eraseMacroScopes
      for prem in premises do
        refs := mergeStructuralTheoremNameSet refs (primitiveRuleAppsInCheckedLFDerivation prem)
      return refs

/-- Primitive rule schemas demanded by one checked theorem's replay tree. -/
def primitiveRuleAppsInCheckedLFTheorem (t : CheckedLFJudgmentTheorem) : NameSet :=
  match t.derivation? with
...
structure StructuralPrimitiveRuleDemandStats where
  ...
  demandedNames : Array Name := #[]
  /-- Lowered primitive rule names not demanded by the current theorem, in signature order. -/
  undemandedLoweredNames : Array Name := #[]
  deriving Inhabited, Repr, BEq
...
def renderStructuralReplayNameList (names : Array Name) : String :=
  if names.isEmpty then
    "none"
  else if names.size <= 8 then
    String.intercalate ", " (names.toList.map toString)
  else
    let head := names.extract 0 4 |>.toList.map toString
    let tail := names.extract (names.size - 4) names.size |>.toList.map toString
    s!"{String.intercalate ", " head}, ... (+{names.size - 8}), " ++
      s!"{String.intercalate ", " tail}"

/-- Render primitive-rule demand counters for a structural replay signature. -/
def renderStructuralPrimitiveRuleDemandStats
    (stats : StructuralPrimitiveRuleDemandStats) : String :=
  s!"primitive-rules considered={stats.considered}, demanded={stats.demanded}, " ++
    s!"lowered={stats.lowered}, demanded_names=\
      {renderStructuralReplayNameList stats.demandedNames}, undemanded_lowered=\
      {renderStructuralReplayNameList stats.undemandedLoweredNames}"

/-- Render theorem-schema demand counters for a structural replay signature. -/
def renderStructuralTheoremSchemaFilterStats
    (stats : StructuralTheoremSchemaFilterStats) : String :=
  s!"theorem-schemas considered={stats.considered}, demanded={stats.demanded}, " ++
    s!"lowered={stats.lowered}, demanded_names=\
      {renderStructuralReplayNameList stats.demandedNames}, undemanded_lowered=\
      {renderStructuralReplayNameList stats.undemandedLoweredNames}"

/-- Render all replay-signature demand filters without lowering any schema. -/
def renderStructuralReplaySignatureFilterSummary (rules : Array CheckedLFRuleSchema)
    (theorems : Array CheckedLFJudgmentTheorem)
    (theoremFilter : StructuralTheoremSchemaFilter)
    (primitiveRuleFilter : StructuralPrimitiveRuleSchemaFilter) : String :=
  let primitiveStats := structuralPrimitiveRuleDemandStats rules primitiveRuleFilter
  let theoremStats := structuralTheoremSchemaFilterStats theorems theoremFilter
  s!"input_sizes=primitive_rules={rules.size}, judgment_theorems={theorems.size}, " ++
    s!"primitive_include_all={primitiveRuleFilter.includeAllForCompatibility}, " ++
    renderStructuralPrimitiveRuleDemandStats primitiveStats ++ ", " ++
    s!"theorem_include_all={theoremFilter.includeAllForCompatibility}, " ++
    renderStructuralTheoremSchemaFilterStats theoremStats

/-- Emit a bounded replay-signature filter line before structural signature lowering. -/
def emitStructuralReplaySignatureFilterStart (site : String) (theoryName : Name)
    (theoremName : Name) (rules : Array CheckedLFRuleSchema)
    (theorems : Array CheckedLFJudgmentTheorem)
    (theoremFilter : StructuralTheoremSchemaFilter)
    (primitiveRuleFilter : StructuralPrimitiveRuleSchemaFilter) : CoreM Unit := do
...
def kernelLFReplayContextOfTheoremsToK (theorems : Array CheckedLFJudgmentTheorem) :
  ...
  pure {
    ...
    theorems := theoremEntries.reverse }
...
/-- Check a structural-kernel replay artifact with prevalidated signature/context indices. -/
def checkStructuralKernelReplayWithValidated (label : String) (signature : Kernel.Signature)
    (context : Kernel.KernelLFCheckContext) (validatedSignature : Kernel.ValidatedSignature)
    (validatedContext : Kernel.ValidatedReplayContext) (statement : Kernel.Judgment)
    (derivation : Kernel.KernelLFDerivation) : CoreM Kernel.CheckedKernelLFDerivation := do
  match Kernel.CheckedKernelLFDerivation.ofReplayValidated signature context validatedSignature
      validatedContext statement derivation with
  | .ok checked => pure checked
  | .error err =>
      throwError "Phase-5c structural kernel replay failed for {label}: {err}"
...
def validateIncrementalLFTheoremKernelReplay (sig : HLSignature) (checked : CheckedSignature)
  ...
  let filterSummary := renderStructuralReplaySignatureFilterSummary checked.lfRuleSchemas
    checked.lfJudgmentTheorems theoremFilter primitiveRuleFilter
  emitStructuralReplaySignatureFilterStart "compact_signature" sig.name t.name
    checked.lfRuleSchemas checked.lfJudgmentTheorems theoremFilter primitiveRuleFilter
...
def validateIncrementalLFTheoremKernelReplayWithCache (cache : CompiledLFCheckCache)
  ...
  let theoremFilter := structuralTheoremSchemaFilterForTheorem t
  let primitiveRuleFilter := structuralPrimitiveRuleSchemaFilterForTheorem t
  let filterSummary := renderStructuralReplaySignatureFilterSummary cache.checkedRuleSchemas
    cache.lfJudgmentTheorems theoremFilter primitiveRuleFilter
  emitStructuralReplaySignatureFilterStart "cached_compact_signature" cache.theoryName t.name
    cache.checkedRuleSchemas cache.lfJudgmentTheorems theoremFilter primitiveRuleFilter
  let structuralSig ← liftStructuralKernelExcept
  ...
  let validatedReplayCtx ← liftStructuralKernelExcept
    s!"judgment_theorem '{t.name}' cached compact validated replay context" <|
      Kernel.ValidatedReplayContext.ofContext structuralReplayCtx
  let structuralAssumptions ← liftStructuralKernelExcept
  ...
  let localParameters := t.binders.toList.map (fun b => Kernel.KLocalName.ofName b.name)
  ...
  let (_structuralDeriv, _structuralStmt, checkedStructuralReplay, replayMode) ←
    try
      ...
      let structuralDerivExpanded ← lowerLFDerivationToStructuralKernelWithMode cache.checkedHL
        ...
        structuralSigExpanded shallowDeriv
      let structuralStmtExpanded := Kernel.KernelLFDerivation.statement structuralDerivExpanded
      let structuralExpandedReplayCtx ← liftStructuralKernelExcept
        s!"judgment_theorem '{t.name}' expanded cached local replay context" <|
          validatedReplayCtx.withLocalFrame localParameters structuralExpandedAssumptions
      let checkedStructuralReplay ← checkStructuralKernelReplayWithValidated
